...
module.exports = class EPLogger {
	...
	constructor(options = {}) {
		...
		this.file = (options.file !== undefined ? options.file : 'application');
	...
	async write(tag, msg) {
		...
		const dateMinutes = EPLogger.pad(date.getMinutes(), 2);
		const dateSeconds = EPLogger.pad(date.getSeconds(), 2);
